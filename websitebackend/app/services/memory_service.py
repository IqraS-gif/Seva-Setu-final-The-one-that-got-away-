...
from app.services.firebase_service import db, get_user_reports_cached, get_user_issues, get_issue_cached
...
def get_hybrid_context(user_email: str, query: str, group_id: str = None, group_label: str = None, top_k: int = 5):
    ...
    if group_id:
        ...
        issue_data = get_issue_cached(group_id)
        if issue_data:
            report_ids = issue_data.get("reports", [])
            label = (group_label or issue_data.get("label", "")).lower()
            print(f"[MEMORY] Focus Mode: Issue '{label}' with {len(report_ids)} linked reports.", flush=True)
            
            # STRICT FILTERING: Only use reports explicitly linked to this issue group
            # This prevents "Potholes" showing up in "Anemia" reports just because of same location
            filtered_reports = [r for r in all_reports if r.get("id") in report_ids]
            
            # EMERGENCY FALLBACK: If NO reports are linked yet, use high-precision keyword matching
            if not filtered_reports:
                noise_words = {"survey", "problem", "report", "issue", "initial", "final", "nagar", "ward", "shivaji", "pune", "mumbai", "india", "peth"}
                keywords = [
                ...
                filtered_reports = [
                    ...
                    if any(k in (r.get("primary_category") or "").lower() or k in (r.get("executive_summary") or "").lower() for k in keywords)
                ]
    ...
    for r in all_reports:
        ...
        # FINAL HYBRID SCORE
        # 80% semantic, 10% severity, 10% recency
        final_score = (sem_score * 0.8) + (sev * 0.1) + (time_score * 0.1)
        
        # D. Topic Guardrails (HARD EXCLUSION)
        # Prevents "Potholes" showing up in "Maternal Health" queries
        q = (query or "").lower()
        r_cat = (r.get("primary_category") or "").lower()
        
        health_keywords = {"health", "maternal", "pregnant", "anganwadi", "ambulance", "hospital", "doctor", "medical", "women", "postnatal"}
        infra_keywords = {"pothole", "road", "pipe", "water", "infrastructure", "sanitation", "drainage"}
        
        is_health_query = any(hk in q for hk in health_keywords)
        is_infra_query = any(ik in q for ik in infra_keywords)

        # HARD TOPIC LOCK: If the user asks about health, DISCARD non-health reports
        if is_health_query and r_cat in {"infrastructure", "water", "sanitation"}:
            continue
            
        # HARD TOPIC LOCK: If user asks about infra, DISCARD health reports
        if is_infra_query and r_cat == "health":
            continue
        if group_id:
    ...
    for m in scored_memories:
        ...
        if subject:
            ...
            # Enhanced Fingerprint: Subject + Area + Summary Snippet
            # This ensures that separate maternal surveys in the same area stay distinct
            summary_snip = (r.get("executive_summary") or "")[:50].lower()
            fingerprint = f"data:{(r.get('citizen_name') or '').lower()}|{(r.get('primary_category') or '').lower()}|{(r.get('precise_location') or '').lower()}|{summary_snip}"
            
        # Only keep the highest scoring version of this specific incident
        # Confidence Threshold: Only return memories that are genuinely relevant (0.2+)
        if fingerprint not in seen_fingerprints and m["score"] > 0.2:
