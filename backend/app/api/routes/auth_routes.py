...
async def verify_passport(citizen_id: str):
    ...
    if not user_doc.exists:
        ...
        for doc in docs:
            # Check if doc.id matches uid or starts with uid (case insensitive)
            if doc.id.upper() == uid.upper() or doc.id.upper().startswith(uid.upper() if len(uid) >= 8 else "NOT_ENOUGH_CHARS"):
                user_doc = doc
                break
