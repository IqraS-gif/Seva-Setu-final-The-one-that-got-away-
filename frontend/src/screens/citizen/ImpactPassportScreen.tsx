    ...
    const fetchHistory = async () => {
      try {
        ...
        if (response.data.success) {
          // Store all reports to show both active and completed
              ...
              return (
                <View key={item.id || index} style={[globalStyles.card, styles.historyItem]}>
                  ...
                  <View style={styles.historyTextContainer}>
                    ...
                    <Text style={styles.dateText}>{item.resolved_at || (isResolved ? t('citizen.impactPassport.completed') : 'Pending')}</Text>
                  </View>
                  {isResolved ? (
