...
export const CitizenReportDetailScreen = () => {
  ...
  const getUrgencyColor = () => {
    if (report.status === 'Resolved') return colors.success;
    if (report.status === 'Verified' || report.status === 'In Progress') return colors.accentBlue;
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <SectionTitle title="Updates on Complaints" />
        <View style={[globalStyles.card, styles.feedbackCard]}>
           <Text style={styles.feedbackText}>
             {report.status === 'Resolved' 
               ? "Your issue has been resolved! Please check the details above."
               : report.status === 'Verified' || report.status === 'In Progress'
               ? "A volunteer has verified this issue and is working on it. Stay tuned!"
