                ...
                <>
                  ...
                  {(filteredData.pending || []).map((assign) => (
                    <MissionCard
                      ...
                      volunteerName={volunteerProfile?.name || 'Volunteer'}
                    />
                  ))}
                  ...
                  {(filteredData.live || []).map((match) => (
                    <MissionCard
                      ...
                      onAccept={async () => {
                         ...
                         await joinMatch(match.event_id, volunteerId, 'accepted', match.match_score, match.score_breakdown, match.supervisor_id, match.supervisor_name);
                      }}
                      onDecline={async () => {
                         await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                         await joinMatch(match.event_id, volunteerId, 'declined', match.match_score, match.score_breakdown, match.supervisor_id, match.supervisor_name);
                      }}
                      onViewReasoning={() => {
                        Haptics.selectionAsync();
...
const MissionCard = ({ data, isDirect, currentSkills, onAccept, onDecline, onViewReasoning, navigation, volunteerId, volunteerName }: {
  data: any;
  isDirect: boolean;
          ...
          {isDirect ? (
            ...
            <TouchableOpacity 
              onPress={() => navigation.navigate('Chat', {
                ...
                recipient_name: data.supervisor_name || 'Supervisor',
