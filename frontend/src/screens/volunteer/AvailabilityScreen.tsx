...
export const AvailabilityScreen = ({ navigation }: any) => {
  ...
  const monthsTrans = t('calendar.months', { returnObjects: true });
  const MONTHS = Array.isArray(monthsTrans) ? monthsTrans : ['January','February','March','April','May','June','July','August','September','October','November','December'];
  
  const daysTrans = t('calendar.days', { returnObjects: true });
  const DAYS_OF_WEEK = Array.isArray(daysTrans) ? daysTrans : ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
