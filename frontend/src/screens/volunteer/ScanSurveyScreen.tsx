...
export const ScanSurveyScreen = () => {
  ...
  }, [selectedCategory, reportSourceFilter, reports]);
  ...
  const processBatch = async (files: { uri: string, type: string, name: string }[]) => {
    ...
    try {
      ...
      for (const file of files) {
        console.log(`[ScanSurvey] Processing file: ${file.name}, type: ${file.type}`);
        ...
        const response = await fetch(`${API_BASE_URL}/scan-form`, {
          method: 'POST', 
          body: formData,
          // IMPORTANT: Do NOT set Content-Type header manually for multipart/form-data with FormData.
          // The fetch API will automatically set it with the correct boundary.
        });

        if (!response.ok) {
          const errorText = await response.text();
          console.error(`[ScanSurvey] Server error (${response.status}):`, errorText);
          throw new Error(`Server returned ${response.status}: ${errorText}`);
        }

        const result = await response.json();
        console.log('[ScanSurvey] Response received:', JSON.stringify(result).substring(0, 200) + '...');
        
        if (result.success) {
          results.push({ parsed: result.parsed_data, file, url: result.url, publicId: result.public_id });
        } else {
          console.warn('[ScanSurvey] Analysis returned success=false:', result);
          throw new Error(result.error || 'AI analysis failed to produce structured data.');
        }
      }
      ...
      console.error('[ScanSurvey] processBatch failed:', error);
      Alert.alert(
        'Analysis Failed', 
      ...
      <View style={{ flex: 1 }}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.premiumCard}>
            ...
            <FormField label={t('volunteer.scan.executiveSummary')} value={getBilingualText(parsed.executive_summary, language)} onChangeText={(t: string) => updateField('executive_summary', t)} multiline />
            <FormField label={t('volunteer.scan.primaryCategory')} value={getBilingualText(parsed.primary_category, language)} onChangeText={(t: string) => updateField('primary_category', t)} />
            <FormField label={t('volunteer.digitalSurvey.subCategory')} value={getBilingualText(parsed.sub_category, language)} onChangeText={(t: string) => updateField('sub_category', t)} />
            <FormField label={t('volunteer.digitalSurvey.durationOfProblem')} value={getBilingualText(parsed.duration_of_problem, language)} onChangeText={(t: string) => updateField('duration_of_problem', t)} />
            <FormField label={t('volunteer.scan.urgencyLevel')} value={getBilingualText(parsed.urgency_level, language)} onChangeText={(t: string) => updateField('urgency_level', t)} />
          </View>
          ...
          <View style={styles.premiumCard}>
            ...
            <FormField label={t('volunteer.digitalSurvey.vulnerabilityFlag')} value={getBilingualText(parsed.vulnerable_group, language)} onChangeText={(t: string) => updateField('vulnerable_group', t)} placeholder="women / children / elderly / disabled" />
          </View>
          ...
          <View style={styles.premiumCard}>
            ...
            <FormField
              ...
              value={getBilingualArray(parsed.expected_resolution_timeline, language).join('\n')}
            ...
            <FormField
              ...
              value={getBilingualArray(parsed.detailed_resolution_steps, language).join('\n')}
              onChangeText={(t: string) => updateField('detailed_resolution_steps', t.split('\n').filter(s => s.trim()))}
              multiline
              placeholder="Step 1: ...\nStep 2: ..."
            />
            <FormField label={t('volunteer.scan.govtScheme')} value={getBilingualText(parsed.govt_scheme_applicable, language)} onChangeText={(t: string) => updateField('govt_scheme_applicable', t)} />
            <FormField label={t('volunteer.scan.aiRecommendations')} value={getBilingualText(parsed.ai_recommended_actions, language)} onChangeText={(t: string) => updateField('ai_recommended_actions', t)} multiline />
          </View>

          {/* Section 5: Qualitative */}
          <View style={styles.premiumCard}>
            <Text style={styles.formSectionTitle}>5. {t('volunteer.scan.communityVoice')}</Text>
            <FormField label={t('volunteer.scan.historicalInsights')} value={getBilingualText(parsed.previous_complaints_insights, language)} onChangeText={(t: string) => updateField('previous_complaints_insights', t)} multiline />
            <FormField label={t('volunteer.scan.extractedKeywords')} value={getBilingualArray(parsed.key_complaints, language).join(', ')} onChangeText={(t: string) => updateField('key_complaints', t.split(',').map(s => s.trim()))} />
            <FormField label={t('volunteer.digitalSurvey.communitySentiment')} value={getBilingualText(parsed.sentiment, language)} onChangeText={(t: string) => updateField('sentiment', t)} />
            <FormField label={t('volunteer.digitalSurvey.detailedDescription')} value={getBilingualText(parsed.description, language)} onChangeText={(t: string) => updateField('description', t)} multiline />
          </View>
      ...
      >
        <BottomSheetScrollView contentContainerStyle={{ paddingHorizontal: spacing.xl, paddingBottom: spacing.xxl }}>
                      ...
                      <View style={{ marginBottom: spacing.md }}>
                        ...
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 8 }}>
                          {displayedReport.photo_url && (
                            <TouchableOpacity onPress={() => { setViewerUri(displayedReport.photo_url || null); setViewerVisible(true); }} style={styles.attachmentImageWrapper}>
                          ...
                          {displayedReport.media_attachments?.filter((m: any) => m.type === 'image').map((img: any, idx: number) => (
                            <TouchableOpacity key={idx} onPress={() => { setViewerUri(img.url || null); setViewerVisible(true); }} style={styles.attachmentImageWrapper}>
                ...
                <View style={[styles.sectionCard, { borderLeftColor: colors.primarySaffron }]}>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                    {[...(displayedReport.media_attachments || []), ...(displayedReport.photo_url ? [{ url: displayedReport.photo_url, type: 'image' }] : [])].map((img, idx) => (
                      <TouchableOpacity key={idx} onPress={() => { setViewerUri(img.url || null); setViewerVisible(true); }} style={{ width: '31%', aspectRatio: 1, borderRadius: 12, overflow: 'hidden' }}>
          ...
          <View style={{ gap: spacing.sm, marginTop: spacing.md, marginBottom: spacing.xl }}>
            {displayedReport && displayedReport.status !== 'Resolved' && displayedReport.status !== 'Completed' && (
              <TouchableOpacity
                activeOpacity={0.8}
